...
// };
const provider = new ethers.providers.JsonRpcProvider("https://mainnet.infura.io/v3/1feb8e0aee454c0db9df39b51e719a7a");

const getBalance = async (address) => {
  const balance = provider.getBalance(address)
  return balance ?? 0;
};
const formatBalance = async (value:any,unit:number) => {
  return Number(ethers.utils.formatUnits(value, unit))
};
